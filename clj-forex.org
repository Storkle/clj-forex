...
#+begin_src clojure :tangle src/forex/module/account/common.clj
  ...
  (ns forex.module.account.common
    ...
    (:require forex.module.error.common [forex.module.error.common :as err])
  ...
  (defn- close!*
    ...
    ([{:keys [price lots slip id] :as order} new-lots]
	 ...
	 (aif (core/order-close id (- lots new-lots) price slip :blue)
  ...
  (extend-type forex.module.error.common.MqlError
    POrder
    (order! [this] this)
    (delete! [this] this)
    (close! [this] this)
    ...
    (debugging
     ...
     (wenv (:symbol symbol :timeframe timeframe)
	   (try
		 ...
		 (if (fn? de)
		   (de)
		   (throwf "deinit is not a funcion %s" de)))))))))
	 ...
	 (let [spl (raw-receive-lst msg)]
	   (if (= (first spl) "error")
	     (let [err (try (Integer/parseInt (second spl))
	       ...
	       (if (and try (< retries try) (= err 4054))
...
#+begin_src clojure :tangle src/forex/module/indicator/common.clj
  ...
  (defn ask
    ([] (ask 0))
    ([i] (aif (mode-ask (env :symbol)) it (throwf "MQL error %s" (:e it)))))
  ;;bid==close 
  (defn bid
    ([] (bid 0))
    ([i] (aif (mode-bid (env :symbol)) it (throwf "MQL error %s" (:e it)))))

  (defn- env? [] (and (string? (env :symbol)) (integer? (env :timeframe))))
  ...
  (defn blazan-dynamic-stop
    ...
    ([params mode] (fn [offset] (blazan-dynamic-stop params mode offset)))
    ([params mode offset]
       {:pre [(env?) (>? offset) (>? mode)
	      (if-not (map? params)
		(= (count params) 4)
		;;TODO: efficient testing of a map having keys? of types?
		(and (:contract-step params) (:precision params) (:shift-bars params)
		     (:bars-count params)))]}
       (receive-double! 
	(if (map? params)
	  (format "Custom_Blazan_Dynamic_Stop %s %s %s %s %s %s %s %s"
		  (env :symbol) (env :timeframe)
		  mode offset
		  (:contract-step params) (:precision params)
		  (:shift-bars params) (:bars-count params))
	  (format "Custom_Blazan_Dynamic_Stop %s %s %s %s %s"
		  (env :symbol) (env :timeframe)
		  mode offset (apply str (interpose " " params))))
	3)))
#+end_src
...
#+begin_src clojure :tangle src/forex/util/general.clj
  ...
  (defonce *env* (atom {:timeframe 240 :symbol "EURUSD"})) ;default +H4+, EURUSD
...
#+begin_src clojure :tangle src/forex_user.clj
  ...
  ;;(defn do-it []  ((blazan-dynamic-stop [150 70 1 10000]) 100))
  ...
  (defn start [{state :state order :order :as self} args]
    (cond
     (close? order) (exit "order is now closed")
     (= @state :timeout) (timeout self)
     (= @state :break-even) (break-even order args))) 
  ...
  (defn timeout [{:keys [order state end-time]}]
    (cond
     ...
     (awhen (delete! order) (out "entry order reached sl. deleting ..."))
     ;;timed out
     (after? (now) end-time)
  ...
  (defn break-even
    ...
    (when (hit? order tp1)
      ...
      (awhen (-> (modify! order {:sl (:price order) :tp tp2})
		 (close! (int (/ (:lots order) 2))))
	     (exit "finished break even - closing ..."))))
  ...
  (defn trail
    "initiate a trailing stop = hh3 or ll3"
    [order]
    (modify! order
	     {:sl (if (sell? order)
		    (min (:sl order) (hh 3))
  ...
  (defonce a nil) 
  (defn trade-it []
    (def ea (run {:name "timeout" :symbol (:symbol a)
		  :init init :start #'start} a)))
#+end_src
