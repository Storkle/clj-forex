...
#+begin_src clojure :tangle src/forex/util/emacs.clj
  ...
  (defn add-hooks [hook functions] (doall (map #(push hook %) functions)))
...
#+begin_src clojure :tangle src/forex/module/account/utils.clj
  ...
  (defn lot 
    ...
    ([num symbol]
       (let [a (to-big num)
	     b (to-big (or val (mode-minlot symbol)))]
	 (* b (.intValue (/ a b))))))
...
#+begin_src clojure :tangle src/forex/module/ea.clj
  ...
  (defn save-eas
    ...
    ([save-file]
       (debugging "Save EA:"
	(try
	  (info "Attempting to acquire writeLock of *ea-pre-lock* ...")
	  (locking-write-timeout [*ea-pre-lock* 1000] 
				 (info "saving ...")
...
#+begin_src clojure :tangle src/forex/examples/timeout_ea/timeout_ea.clj
  ...
  (defn trail [_]
    (modify! order {:sl (omax (:sl order) (if (sell? order) (hh 3) (ll 3)))})) 
  ;;;GUI
  (defn init-gui []
