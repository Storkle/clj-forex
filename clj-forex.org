...
#+begin_src clojure :tangle src/forex/util/mql_indicator_devel.clj
  ...
  (defn extract [args]
    (let [type-to-fn (fn [type]
	  ...
	  user-extract (apply str
			      (interpose "\n"
					 (for+ [[type name] args index (iterate inc 7)]
  ...
  (defn to-test
    ...
    ([all name]
       (format "%nint %s(string c,string command[]) {%n%s%nelse {%nreturn(-1);%n}%nreturn(0);%n}"
	       name
	       (apply str
		      "if (c==\"0\") {\n}\n"
  ...
  (defn iCustom
	...
	(let [extract (extract args)
	      custom
	      (format 
	       "array[j] = iCustom(symbol,timeframe,\"%s\",%smode,shift);"
	       (name-of f) (listify (map second args)))
	      body (format "double array[];%nArrayResize(array,amount);int j=0;%n
  ...
  send_array(array); 
	      ...
	      function-header (format "void %s (string command[]) {"
				      function-name)]  
	  {:file f :file-name name
	   :function-name function-name :args args :buffers buffers
	   :code  (format "%s %s %s %s" function-header extract body return)}))))
