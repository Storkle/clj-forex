...
#+begin_src emacs-lisp
  (defvar clj-forex-file)
  (setq clj-forex-file "/home/seth/Dropbox/.rep/clj-forex/clj-forex.org")
        ...
        (with-cd (file-name-directory org-file)
                 (dolist (file out)
                   (with-file
                    ...
                    (save-excursion (org-babel-detangle)))))
        (message "Detangled %s source blocks" (length out)))))
...
#+begin_src clojure :tangle src/forex_user.clj
  ...
  (defn break-even
    ...
    (when (hit? order tp1)
      (out "closing to half ...")
      (close! order (int (/ (:lots order) 2)))
      (modify! order {:sl (:price order) :tp tp2})
