...
#+begin_src clojure :tangle src/forex/module/account.clj
  ...
  (defn lot
    ...
    ([num symbol] (round num (decimal-places (mode-minlot symbol)))))
#+end_src
*** Ea
#+srcname:ea-common
#+begin_src clojure :tangle src/forex/module/ea.clj
  ...
  (defonce *ea* nil)
  (defonce *args* nil)
  (defmacro with-ea [[name ea] & body]
    `(let [ea# ~ea]
       (debugging (format "%s %s:" ~name (:name ea#))
    ...
    (with-ea ["" ea]
	 ...
	 (finally
	  ...
	  (catch-unexpected
       ...
       (clojure.contrib.error-kit/handle *ea-stop* []))))
  ...
  (defn new-ea
    ([] (new-ea {}))
    ([{:keys [ns symbol timeframe run args] :or {symbol (env :symbol)
						 run run-by-tick
	 ...
	 (let [name (str (ns-name ns))
	   ...
	   (EA. (format "%s %s, %s" name symbol timeframe)
		init deinit start symbol timeframe (or args {})
		nil run (ns-vars ns))))))
  ...
  (defn run
    ...
    ([args] (run *ns* args))
    ([ns args]
       (run-all (new-ea {:ns ns :args args :symbol (:symbol args)
			 :period (:period args)}))))

  (defn sym [] (:symbol *ea*))
  (defn period [] (:timeframe *ea*))
...
#+begin_src clojure :tangle src/forex/util/general.clj
  ...
  (defn throwf [msg] (raise *clj-forex-error* msg))
