...
public class FantailVMA extends Indicator {
	...
	//TODO: make error message Attempting to access sequence with min/max of 0/1 at index 2 better
   ...
   //TODO: option between having default values? thats what metatrader does. even for price streams! but it is convenient to do it my way for debugging...
	public int Execute() {
		int limit = limit();
		if (isStart()) {
			for (int i=limit;i>limit-adx_period;i--) {
				VarMA.set(i,close(i));MA.set(i,close(i));STR.set(i,high(i)-close(i));
				sPDI.set(i,0);sMDI.set(i,0);ADX.set(i,0);ADXR.set(i,0);			
			}
		}
